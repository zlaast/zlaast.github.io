	...
	async function search() {
		...
		database.forEach((entry) => {
			if (
				...
				entry.subtitle.toLowerCase().includes(searchInput.toLowerCase())
			)
				results.add(entry)
		})
...
{#await fetchDatabase then db}
	<div class="flex flex-col justify-center">
		<div class="w-[50vw] max-md:w-[75vw]">
			<input
				type="search"
				class="z-50 mt-8 h-14 w-[50vw] max-md:w-[75vw] bg-[#27292d] pl-11 text-left text-neutral-300"
				placeholder="Search"
			...
			<ul id="page-results-list" class="z-50 bg-[#27292d] px-3 text-neutral-300 outline-none">
				{#each output as result}
					<li class="hover:bg-bg mx-3 hover:text-[#cba0e7] list-none">
						<a
							class="flex p-4 pl-5 hover:text-[#cba0e7] text-neutral-300 outline-none"
							href="/blog/{result.url}">{result.title} - {result.subtitle}</a
						>
					</li>
				{/each}
			</ul>
