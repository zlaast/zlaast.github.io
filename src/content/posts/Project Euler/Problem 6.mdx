...
<Math inline="false" equation="\sum\limits_{x=1}^{n}x^2 = \dfrac{ n(n+1)(2n+1) }{ 6 }" />


And that's it. Since <Math equation="n = 100" />, then
<Math inline="false" equation="\sum\limits_{x=1}^{100}x^2 = \dfrac{ 100 \times 101 \times 201 }{ 6 }" />

<br/>
### Square of Sums

The sum of all natural numbers from 1 to <Math equation="n" /> is
<Math inline="false" equation="\sum\limits_{1}^{n}n = \dfrac{ n(n+1) }{ 2 }" />

We just have to use this formula, then square the sum at the end!

Since <Math equation="n = 100" />, we'll get
<Math inline="false" equation="\left[\dfrac{ 100 \times 101 }{ 2 } \right]^2 = \dfrac{ 100 \times 101 \times 100 \times 101 }{ 4 }" />
...
<Tabs list={
    [
        {
            "tab": "problem06.py",
            "url": "https://github.com/zlaast/Project-Euler/blob/main/Problems%20%5B1-100)/problem06.py"
        },
        {
            "tab": "main.rs",
            "url": "https://github.com/zlaast/Project-Euler/blob/main/Problems%20-%20Rust/problem_6/src/main.rs"
        },
    ]
}>
