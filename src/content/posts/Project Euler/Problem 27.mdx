...
excerpt: "Why does #3 use only positive primes? Actually, it doesn't have to, but from observing the output, only the positive prime numbers produce chains, so we can do away with trying the negative side."
read_time: 2
---
...
- `soe()` is the Sieve of Eratosthenes, a prime number sieve written in [Problem 3](/posts/project-euler/problem-3)
