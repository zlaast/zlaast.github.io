...
For example, the number is 2595 can be created using the prime numbers (factors) 3, 5, and 173. That is,

<Math inline="false" equation="3 \times 5 \times 173 = 2595" />

To obtain a list of prime factors for a number <Math equation="x" />, we must

1. Create a list of prime numbers
2. Repeatedly and **evenly** divide <Math equation="x" /> by those primes until it is no longer possible
...
The list is of length <Math equation="n + 1" /> (`n+1` is used to account for the zeroth index).
...
**Note**: I say *repeatedly* divide, as a number can have multiple of the same prime factor. For example, <Math equation="27 = 3 \times 3 \times 3" />.
