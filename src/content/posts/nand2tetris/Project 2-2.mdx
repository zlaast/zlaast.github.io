...
excerpt: "This is the continuation of the previous post, Project 2 - Boolean Arithmetic. In this post, only the Arithmetic Logic Unit (ALU) is created, but it is deserving of its own writeup as there is a lot packed into this section that needs examining."
read_time: 10
---
...
<div class="text-center py-3">$$ x + 1 = \overline{(\overline x + y)} $$</div>

Just to be explicit, let's substitute `y` with `-1` and get

<div class="text-center py-3">$$ x + 1 = \overline{ ( \overline x - 1 ) } $$</div>
...
<div class="text-center py-3">$$ -z = \overline z + 1 $$</div>
...
<div class="text-center py-3">$$ x - y = x + (\overline y + 1) $$</div>

But what we actually did in the course was,

<div class="text-center py-3">$$ x - y = \overline{ \overline x + y } $$</div>
...
<div class="text-center py-3">$$ \overline z = -z - 1 $$</div>
...
<div class="text-center py-3">$$ x - y = -[-x + y] $$</div>
...
<div class="text-center py-3">$$ y - x = \overline { x + \overline y } $$</div>
...
<div class="text-center py-3">$$ \overline { \overline x \land \overline y } = x \lor y $$</div>

This was explored in [the first post of the series](/posts/nand2tetris/project-1#or-gate), but this is just _De Morgan's Theorem_.
...
<Tabs>
<Tab name="ALU.hdl" checked="checked" group="1">
```
CHIP ALU {
    IN
