...
In assembly, the `div` instruction divides the `rax` register by either another register or a value in memory.
You also *cannot* divide by an immediate! 
